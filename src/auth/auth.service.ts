...
export class AuthService {
  ...
  async googleLogin(user) {
    const { email, name, socialId, deleted, pictureUrl, id } = user; // user.serivce의 구글로그인에서 받아온 user값

    const googlePayload = { email, name, socialId, deleted, pictureUrl, id }; // user에서 추출해서 jwt토큰에 넣어줄 정보들
